...
export async function hybridSearch(
  ...
  if (resolvedMode.autocut && offset === 0) {
    ...
    const autocutMinTop = rerankerEmitsNormalizedScores(effectiveRerankerModel)
      ? resolvedMode.autocut_min_top
      : 0;
    const r = applyAutocut(
      returnPool,
      (x) => x.rerank_score,
      // v0.46.15 (#1863) landed the weak-top floor upstream as
      // `autocut_min_top`, so the fork's parallel `autocut_min_top_score` knob
      // retires here. What upstream did NOT bring is the raw-logit gate above:
      // its floor is an unguarded [0,1] threshold. Keep passing the GATED value.
      { enabled: true, jumpRatio: resolvedMode.autocut_jump, minKeep: 1, minTopScore: autocutMinTop },
