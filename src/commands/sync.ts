...
export async function writeSyncAnchor(
  ...
  //
  // Fork-merge note (2026-08-14): upstream #2114 added `repoDir` at this same
  // trailing slot. Both params are kept; repoDir is placed first (matching
  // upstream's position, minimizing future-merge friction) and pullFailed last.
  // The two are independent — repoDir guards the legacy no-sourceId anchor
  // below, pullFailed gates last_sync_at above. Positional callers pass both.
  pullFailed = false,
): Promise<void> {
  if (sourceId) {
    const col = which === 'repo_path' ? 'local_path' : 'last_commit';
