...
async function performSyncInner(engine: BrainEngine, opts: SyncOpts): Promise<SyncResult> {
  ...
  if (lastCommit === headCommit && !versionMismatch && !versionNeverSet && !hasDetachedWorkingTreeChanges) {
    ...
    //
    // Fork-merge note: the `!opts.dryRun` guard is ours (#1430) and is still NOT
    // in upstream — a dry run must stay side-effect free, and upstream's bare
    // `if (opts.sourceId)` stamps the heartbeat during a preview. Guarded by
