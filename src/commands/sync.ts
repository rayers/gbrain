...
async function performSyncInner(engine: BrainEngine, opts: SyncOpts): Promise<SyncResult> {
  ...
  //
  // Fork-merge note: upstream's `pullFailed` replaced our local #1430
  ...
  // Re-verified against upstream v0.46.18.0 — still only the zero-import case.
  ...
  if (lastCommit === headCommit && !versionMismatch && !versionNeverSet && !(importWorkingTree && hasWorkingTreeChanges)) {
    ...
    //
    // Fork-merge note: local #1430 added the `!opts.dryRun` guard before
    // upstream #3583 adopted it. A dry run must stay side-effect free because a
    // bare `if (opts.sourceId)` stamps the heartbeat during a preview. Guarded by
    // test/sync-no-op-touches-last-sync-at.test.ts, which is what caught the
    // regression when this file was taken wholesale from upstream.
  ...
  if ((versionMismatch || versionNeverSet) && lastCommit === headCommit) {
    ...
    return await performFullSync(engine, fullSyncRoots, headCommit, opts, pullFailed);
  }
...
async function performFullSync(
  ...
  headCommit: string,
  opts: SyncOpts,
  // #1430 (fork): a full reimport triggered by a FAILED pull must not stamp
  // a fresh heartbeat either. See writeSyncAnchor's COALESCE note — first_sync
  // still establishes last_sync_at; only the ADVANCE is suppressed.
  pullFailed = false,
): Promise<SyncResult> {
  ...
  const advanceFull = async (): Promise<void> => {
    ...
    await writeSyncAnchor(engine, opts.sourceId, 'last_commit', headCommit, newestCommitMs(gitContextRoot), gitContextRoot, pullFailed);
    await engine.setConfig('sync.last_run', new Date().toISOString());
    await writeSyncAnchor(engine, opts.sourceId, 'repo_path', anchorPath);
    await writeChunkerVersion(engine, opts.sourceId, String(CHUNKER_VERSION));
  };
